...
async def browse_curated_blocks(
    ...
    """
    # The query param is case-insensitive, as is the curated side (nothing
    # enforces the casing seed data is written with). Instrument categories are
    # always stored lowercase, so that side compares directly — wrapping the
    # column in lower() would make ix_instruments_instrument_category unusable.
    ...
    denom_result = await session.exec(
        select(func.count(func.distinct(Instrument.user_id))).where(
            Instrument.instrument_category == instrument_lower,
