...
The committed response of a mutation, keyed by a client-supplied `Idempotency-Key`, so a retry of a request whose response was lost in flight replays the original result instead of creating a duplicate. See §4 "Idempotent creates".

| Column | Type | Constraints | Notes |
|--------|------|-------------|-------|
| id | serial | PK | |
| user_id | int | FK → users, not null | Covered by the unique constraint below; no index of its own |
| idempotency_key | varchar(255) | not null | Opaque; generated by the client |
| endpoint | varchar(100) | not null | What the key was spent on, e.g. "POST /api/quickstart" |
| request_fingerprint | varchar(64) | not null | SHA-256 of the canonical (validated) request body |
| response_status | int | not null | Status the original request returned |
| response_body | jsonb | not null | The original response, replayed verbatim |
| created_at | timestamptz | not null, indexed | |

Unique constraint: `(user_id, idempotency_key)` — keys are scoped per user, so one account's key can never replay another's result. It's also the lock a concurrent duplicate blocks on.

Retention: no expiry job yet. Rows are a few KB and written once per opted-in mutation (quick start runs about once per user), so growth is negligible. Keys only matter for the minutes-long retry window; `created_at` is indexed so a periodic `DELETE … WHERE created_at < now() - interval '7 days'` is cheap to add when more, or higher-frequency, endpoints adopt this.
...
A `POST` that commits server-side but whose response never reaches the client — dropped connection, timeout, backgrounded tab — is indistinguishable from one that failed, so retrying it creates a second copy of everything. Endpoints that support it accept an optional key:

```
Idempotency-Key: 3f0a9c1e-…       (unique per attempt, chosen by the client)
```

Currently supported by `POST /api/quickstart`. The convention (not a per-endpoint invention) is:

- **Header, not a body field** — it's request metadata, so it stays out of every request schema and any POST can opt in. Matches the IETF draft / Stripe convention.
- **A repeat replays the original response verbatim** — same status code, same body, same ids — plus an `Idempotent-Replay: true` response header. Deliberately *not* a `409`: a client can't act on "you already did this" any differently than on "this failed", which is the problem being solved.
- **A new key creates normally.** Keys are scoped per user, so one client's key can never read another account's result.
- **The same key with a different payload is a `409`.** That's a key reused for a different operation (a client bug), and answering it with the old result would be silently wrong. A retry sends an identical payload; changed answers mean a new attempt and a new key.
- **A rejected request doesn't burn its key.** The key is claimed inside the request's transaction, so a `4xx`/`5xx` rolls it back and the same key can be retried.
- **A blank or over-long (>255 chars) key is a `400`.** Omitting the header is fine — behavior is then exactly as before.
- Concurrent duplicates are safe: the second request waits on the key, then replays the winner's response.

State lives in `idempotency_records` (§3). Client side, the quick-start wizard derives one key per attempt from the payload it's about to send — stable across retries of the same answers, new when an answer changes (`frontend/src/lib/idempotency.ts`).
...
- Accepts an **`Idempotency-Key`** header (see §4 "Idempotent creates"). The wizard sends one per attempt, so the "try again" its error state offers can't create a second instrument, piece and plan when the first request committed and only its response was lost.
