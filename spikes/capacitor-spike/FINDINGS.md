# Capacitor vs. PWA — spike findings (#305)
...
one at a time; raw logs in `evidence/A*.log`.
...
slash still leaves an empty path extension). `capacitor-router-sim.mjs` in this
directory transcribes that function; the observations below ran against it in
headless Chromium (`observed (sim)` — path resolution only, not WKWebView):
...
Screenshot: `evidence/screenshots/protected-signed-in.png`.
...
- **`Info.plist` first, before any mic code** — `observed`,
  `ios/App/App/Info.plist` now carries `NSMicrophoneUsageDescription` and
...
- **Provider abstraction** in `src/lib/voice/` — one interface, a Web Speech
...
…and the generated `ios/App/CapApp-SPM/Package.swift` contains only Capacitor
and Cordova. The plugin is simply absent from the build; JS calls would fail at
runtime, not at build. Re-adding with `--packagemanager CocoaPods` wires it
...
- With `partialResults: true`, `start()` resolves immediately and text arrives
  ...
  last partial *is* the transcript. The provider in `src/lib/voice/` already
  handles this.
...
transcript in `evidence/D1-cors-probes.md`. `observed`.
...
works — implemented and typechecked in `src/lib/voice/` here:
...
- Open the same page in **mobile Safari** on the same phone
  (`npm run build && node capacitor-router-sim.mjs 3200 out`, then
...
- Raw evidence: `evidence/` — build logs per failure, `A9-routing-observations.md`,
  `D1-cors-probes.md`, three screenshots.
...
- Everything here is throwaway. If Capacitor wins, the port starts from
  Kantelo's real code, not from this directory.
